...
uint8_t CAN1_Mode_Init(uint32_t tsjw,uint32_t tbs1,uint32_t tbs2,uint16_t brp,uint32_t mode)
{
    ...
    CAN1_FilerConf.FilterIdHigh=0x0000;  //MyId<<5;     //32位ID wt.edit 2018.05.10
    CAN1_FilerConf.FilterIdLow=0x0000;   //BroadcastId<<5;
    ...
    CAN1_FilerConf.FilterMode=CAN_FILTERMODE_IDMASK;//CAN_FILTERMODE_IDLIST; wt.edit 2018.05.10
    CAN1_FilerConf.FilterScale=CAN_FILTERSCALE_32BIT;//CAN_FILTERSCALE_16BIT;
...
void HAL_CAN_MspInit(CAN_HandleTypeDef* hcan)
{
    ...
    GPIO_Initure.Pin=GPIO_PIN_11;   //PA11,12
    GPIO_Initure.Mode=GPIO_MODE_INPUT ;          //推挽复用
    GPIO_Initure.Pull=GPIO_PULLUP;              //上拉
     HAL_GPIO_Init(GPIOA,&GPIO_Initure);         //初始?
	
	 GPIO_Initure.Pin=GPIO_PIN_12;   //PA11,12
 ...
 *
********************************************************************/
uint8_t CAN1_Send_Msg(uint8_t* msg,uint8_t len,uint32_t stID)
{	
    uint16_t i=0;
    CAN1_Handler.pTxMsg->StdId=stID;        //标准标识符(11位)
    CAN1_Handler.pTxMsg->ExtId=0;        //扩展标识符(29位)CAN1_Handler.pTxMsg->ExtId=exID; 
    ...
    if(HAL_CAN_Transmit(&CAN1_Handler,10)!=HAL_OK) return 0;     //发送
    return 1;		
}
/**********************************************************************
 *
    ...
    for(i=0;i<CAN1_Handler.pRxMsg->DLC;i++)
    	{
			...
			//CAN_RX_buf[i]=CAN1_Handler.pRxMsg->Data[i]; //wt.edit 2018.05.08
    	}
		*id=CAN1_Handler.pRxMsg->StdId; 
