...
{  
  ...
  if(step < 0)      // 步数为负数，向马的的方向移动。
  {
		...
		if((stop_flag==1) && (NewOrigin_flag==0))  //按stop键，执行此语句
		{
		  ...
		  srd.rel_step  =step;
		 // PulseNumbers=DRV8825_Read_CurrentPosition();     //wt.edit 18.03.17
  ...
  {     
	    srd.dir = CW;  // 顺时针方向旋转---向前走，背离马达方向移动。---前进方向
